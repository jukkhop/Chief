...
class Common
{
    ...
    static public function literalSizeToNumeric($v){ 
        $l = substr($v, -1);
        $ret = substr($v, 0, -1);
        switch(strtoupper($l)) {
            case 'P':
    ...
    static public function maxUploadFilesize()
    {
        return min(Common::literalSizeToNumeric(ini_get('upload_max_filesize')), Common::literalSizeToNumeric(ini_get('post_max_size')));
    }
    
    # Transforms an integer (bytes) to an human readable filesize
    static public function humanReadableFilesize($size)
    {
    ...
    static public function humanReadableTime($time, $format = 'H:i:s')
    {        
